...
use crate::{PlatformConfig, PurchasedPaper, ResearchPaperState, UserAccount, ANCHOR_DISCRIMINATOR,ErrorCodes};
...
impl <'info> PurchaseAccess<'info> {
    pub fn purchased_paper_access(&mut self, _uuid:u32,bumps:&PurchaseAccessBumps) -> Result<()> {
        ...
        let publisher_amount = total_amount.checked_sub(platform_fee)
            .ok_or(error!(ErrorCodes::MathOverflow))?;
